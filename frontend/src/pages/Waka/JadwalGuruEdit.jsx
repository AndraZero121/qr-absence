...
import { FaArrowLeft, FaEdit, FaImage, FaCloudUploadAlt, FaSave, FaTimes, FaSpinner } from 'react-icons/fa';
...
function JadwalGuruEdit() {
    ...
    const fetchTeacher = async () => {
      try {
        ...
        if (teacher.schedule_image_path) {
          setPreviewImage(`${STORAGE_BASE_URL}/${teacher.schedule_image_path}`);
        }
      } catch (error) {
  ...
  const handleFileChange = (e) => {
    ...
    if (!validTypes.includes(file.type)) {
      setErrors(prev => ({ ...prev, file: 'Format file harus JPG, PNG, atau GIF' }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.file) {
        ...
        await apiClient.post(`/teachers/${id}/schedule-image`, uploadData, {
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-white/50 flex flex-col sm:flex-row justify-between items-center gap-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-gray-500 font-bold text-sm uppercase tracking-widest mt-3">Perbarui Informasi & Berkas Guru</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 lg:p-14 space-y-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10">
            <div className="space-y-8">
              <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.3em] mb-8 pb-4 border-b border-gray-50">Informasi Dasar</h3>
              {[
                ['nip', 'NIP', 'Contoh: 198...'],
                ['name', 'Nama Lengkap', 'Nama gelar lengkap'],
                ['subject', 'Mata Pelajaran', 'Mata pelajaran utama'],
                ['email', 'Alamat Email', 'Email aktif sekolah'],
                ['phone', 'Nomor WhatsApp', 'Format: 08xx...'],
              ].map(([name, label, placeholder]) => (
                <div key={name} className="space-y-3 group">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 group-focus-within:text-blue-600 transition-colors">{label}</label>
                  <input
                    type="text"
                    name={name}
                    ...
                    className="w-full px-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-bold text-gray-700 placeholder:text-gray-300 shadow-inner"
                  />
                </div>
              ))}
            </div>
